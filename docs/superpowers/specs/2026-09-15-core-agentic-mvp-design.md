...
- Real IMD/INCOIS-specific cyclone/lightning feeds (real global feeds are now wired for both — GDACS for cyclones, a public Blitzortung MQTT bridge for lightning — not IMD/INCOIS specifically; see below)
- A true navigational route planner with land-avoidance, shipping lanes, and bathymetry (a scoped-down Route Safety Agent — a hazard scan over a linearly-interpolated path, not full pathfinding — is now live; see below)
- Voice/SMS/IVR channels
- Real multi-user accounts/auth (a session_id has no owner; anyone holding it can read its history)
- Push notifications reaching a user with no tab open (proactive alerting is now live, but strictly in-app/tab-open only — see below)

**Added post-MVP, no longer deferred (2026-09-15):** real historical SST trend (7-day ERDDAP time series) feeding a basic warming/cooling/stable signal into the ocean-analytics reasoning and the reporting answer; a deterministic geofence warning surfaced prominently in the answer whenever a query resolves near a real nearby protected area; a real chart (SST trend sparkline) in the reasoning-trace panel instead of raw JSON only; **live cyclone alerts via GDACS** (free, global, JTWC-sourced tropical cyclone tracker, filtered to events within 500km of the query location) — this is a real global feed, not the India-specific IMD/INCOIS source the original spec named, but it is live, free, and covers cyclones affecting India's coastline. **Live lightning alerts too** — an earlier version of this document wrongly concluded no free feed existed after the raw Blitzortung.org websocket turned out to be obfuscated; `blitzortung.ha.sed.pl:1883` is a real, free, unauthenticated MQTT broker (maintained by the Home Assistant Blitzortung integration's author) that already republishes the decoded real-time strike feed as clean JSON, subscribed to via geohash-tile topics. This is a real-time sample over a short listen window, not a continuous monitor or historical archive — a strike is only caught if it happens during that window, and an empty result honestly means "none observed just now," not "none exist." **Live geofencing via OpenStreetMap's Overpass API** (free, global, no key) also replaces the old single fake example-polygon MPA snapshot — real named protected areas/sanctuaries near the query location, nearest one within 100km surfaced by name and distance. **Persistent per-session history via SQLite** (not Postgres, deliberate substitution for the same portability reasons as the other free/no-server-process choices in this spec) — the backend now loads and stores conversation history server-side by session_id instead of trusting the client to resend it; this is history persistence only, not real accounts/auth (see the Phase 2 list above). **Proactive hazard alerting** — every 5 minutes, every session with a known location is re-checked with the same weather/risk agents, and an alert is pushed over a long-lived SSE connection on any transition into an unsafe verdict. Scoped explicitly to in-app delivery while a browser tab stays open — no push notifications, no email/SMS; a session with no open tab simply misses the alert. This is a narrower "proactive" than the PS's broader vision, but it is real: unprompted, server-initiated, and demonstrable. **Route Safety Agent** — answers the PS's "safest route" query type via a waypoint hazard scan (5 linearly-interpolated points between two geocoded endpoints, each checked with the real weather/risk agents, with a single perpendicular detour offset attempted around any hazardous waypoint) rather than true pathfinding — an explicit, user-confirmed scope limit (see Phase 2 list above), not a hidden shortcut. Visualized on the map as colored route segments and waypoint markers.
