...
**Added post-MVP, no longer deferred (2026-09-15):** real historical SST trend (7-day ERDDAP time series) feeding a basic warming/cooling/stable signal into the ocean-analytics reasoning and the reporting answer; a deterministic geofence warning surfaced prominently in the answer whenever a query resolves near a real nearby protected area; a real chart (SST trend sparkline) in the reasoning-trace panel instead of raw JSON only; **live cyclone alerts via GDACS** (free, global, JTWC-sourced tropical cyclone tracker, filtered to events within 500km of the query location) — this is a real global feed, not the India-specific IMD/INCOIS source the original spec named, but it is live, free, and covers cyclones affecting India's coastline. Lightning alerts remain a cached, always-empty snapshot — no free REST lightning feed was found (Blitzortung, the standard free community network, requires an authenticated websocket). **Live geofencing via OpenStreetMap's Overpass API** (free, global, no key) also replaces the old single fake example-polygon MPA snapshot — real named protected areas/sanctuaries near the query location, nearest one within 100km surfaced by name and distance. **Persistent per-session history via SQLite** (not Postgres, deliberate substitution for the same portability reasons as the other free/no-server-process choices in this spec) — the backend now loads and stores conversation history server-side by session_id instead of trusting the client to resend it; this is history persistence only, not real accounts/auth (see the Phase 2 list above).
